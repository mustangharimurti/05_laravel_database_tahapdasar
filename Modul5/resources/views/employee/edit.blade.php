...
btn-outline-light my-2 ms-md-auto"><i
    ...
    <div class="container-sm mt-5">
        <form action="{{ route('employees.update', ['employee' => $employee1->employee_id]) }}" method="POST">
            @csrf
            @method('put')
...
alert-dismissible fade show">
                    ...
                    <hr>
                    <input type="hidden" name="id" value="{{ $employee1->id }}" />
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="firstName" class="form-label">First Name</label>
                            <input class="form-control" type="text" name="firstName" id="firstName"
                                value="{{ $employee1->firstname }}" placeholder="Enter First Name">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="lastName" class="form-label">Last
                                Name</label>
                            <input class="form-control" type="text" name="lastName" id="lastName"
                                value="{{ $employee1->lastname }}" placeholder="Enter Last Name">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="email" class="form-label">Email</label>
                            <input class="form-control" type="text" name="email" id="email"
                                value="{{ $employee1->email }}" placeholder="Enter Email">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="age" class="form-label">Age</label>
                            <input class="form-control" type="text" name="age" id="age"
                                value="{{ $employee1->age }}" placeholder="Enter Age">
                        </div>
                    </div>
                        ...
                        <select name="position" id="position" class="form-select">
                            @foreach ($positions1 as $position)
                                <option value="{{ $position->id }}"
                                    {{ $employee1->position_id == $position->id ? 'selected' : '' }}>
